...
namespace nia
{
    ...
    void ls_solver::split_string(std::string &in_string, std::vector<std::string> &str_vec, std::string pattern = " ")
    {
     ...
     void ls_solver::get_objective(std::string in_string)
    {
        std::vector<std::string> vec;
        split_string(in_string, vec);
        std::string obj = delete_pa(vec[1]);
        // std::string obj = delete_pa(vec[vec.size() - 1]);
        // std::cout << obj << std::endl;
        // exit(0);
        if (name2var.find(obj) == name2var.end())
        {
        ...
        obj_vars = name2var[obj];
        if (is_omt && _vars[obj_vars].obj_low_bound == INT32_MAX) _vars[obj_vars].obj_low_bound = INT32_MIN;
        _obj_var_terms.push_back(obj_var_term(obj_vars,1,std::vector<int>{1}));
    }
    ...
    {
        if (t.var_epxs.size() > 3)
        {
            has_unidentified_lits = true;
        }
    ...
    void ls_solver::build_lits(std::string &in_string)
    {
        ...
        if (vec.size() > 2)
        {
            ...
            if (vec[2].size() >= 5)
            {
                ...
                if (str_tmp == "pbge[" || str_tmp == "pbeq[" || str_tmp == "pble[")
                {
                    has_unidentified_lits = true;
                    return;
                }
                ...
                for (; idx < vec.size(); idx++)
                {
                    ...
                    if (vec[idx] == "(")
                    {
                        ...
                        else
                        {
                            ...
                            if (coff > 10000000 || coff < -10000000)
                            {
                                has_unidentified_lits = true;
                                return;
                            }
    ...
    void ls_solver::build_instance(std::vector<std::vector<int>> clause_vec)
    {
        ...
        {
            build_unsat = true;
        } // if there is too many ``if'', then LS should be skipped
            ...
            if (clause_vec[clause_idx].size() == 1)
            {
                ...
                if (l->coff_terms.size() == 1 && is_single_var_term(_terms[l->coff_terms[0].term_idx]))
                {
                    ...
                    if (clause_vec[clause_idx][0] > 0)
                    {
                        ...
                        {
                            new_upper_bound = std::floor((double)(-l->key) / (double)(l->coff_terms[0].coff));
                        } // ax+k<=0   x<=(-k/a)
                        else
                        {
                            new_low_bound = std::ceil((double)(-l->key) / (double)(l->coff_terms[0].coff));
                        } // ax+k<=0  x>=(-k/a)
                    }
                    else
                    {
                        if (l->coff_terms[0].coff > 0)
                        {
                            new_low_bound = std::ceil((double)(1 - l->key) / (double)(l->coff_terms[0].coff));
                        } // ax+k>0 ax+k>=1 x>=(1-k)/a
                        else
                        {
                            new_upper_bound = std::floor((double)(1 - l->key) / (double)(l->coff_terms[0].coff));
                        } // ax+k>=1 x<=(1-k)/a
                    }
    ...
    void ls_solver::reduce_vars()
    {
        ...
        lit *l;
        term new_t;
        // std::cout<<" obj bound: "<<print_128(_resolution_vars[obj_vars].low_bound)<<std::endl;
        ...
        for (uint64_t lit_i = 0; lit_i < _bound_lits.size(); lit_i++)
        {
            ...
            if (l->coff_terms[0].coff < 0)
            {
                ...
                // std::cout<<cur->var_name <<"  "<< print_128(cur->low_bound) << std::endl;
                if (cur->low_bound != -max_int && cur->upper_bound != max_int)
                {
                    if(_bound_clauses[lit_i]>0)
                        l->lits_index = -2;
                    else l->lits_index=-1;
                    cur->obj_low_bound = cur->low_bound;
                }
                else
                {
                    if(cur->obj_low_bound == INT32_MAX) 
                    {
                        // std::cout << cur->var_name << " " << cur->obj_low_bound <<std::endl;
                        cur->obj_low_bound = cur->low_bound;
                        // std::cout << cur->var_name << " " << cur->obj_low_bound <<std::endl;
                    }
    ...
    void ls_solver::reduce_clause()
    {
        ...
        {
            if (v.is_nia && v.upper_bound != max_int && v.low_bound != -max_int)
            {
        ...
        for (variable &v : _vars)
        {
            ...
            if (v.is_nia)
            {
                ...
                v.obj_low_bound = _resolution_vars[transfer_name_to_resolution_var(v.var_name, true)].obj_low_bound;
                _num_nia_vars++;
            }
    ...
    ls_solver::~ls_solver()
    {
        ...
        if (var_in_long_term != NULL)
            delete var_in_long_term;
    }
}
