...
struct lit{
    ...
    int                         is_not=0;//1 means the lit in soft clause is neg lit;
    int                         is_soft=0;//1 means the lit in soft lit;
};
...
struct variable{
    ...
    std::vector<int>            soft_clause_idxs;//new
    std::vector<uint64_t>         soft_literal_idxs;//new
};
...
public:
    //soft new add
    int                       pseudo_hard_clause_weight;
    int                         debug_flag=0;
    void                        test_error();
    void                        print_literal_reduce(lit &l);
    void                        print_formula_with_soft();
    void                        print_term_reduce(term &t);
    std::vector<int>            soft_clauses_weight;
    int                         h_inc;
    int                         is_weighted=0;
    double                          best_time;
    int                         is_sat=0;
    int                         _num_soft_clauses;
    std::vector<int>            _bound_clauses;
    std::vector<std::vector<int>> soft_clauses;
    std::vector<soft_clause>    _soft_clauses;
    int                         _lit_in_unsat_soft_clause_num;
    int                         _bool_lit_in_unsat_soft_clause_num;
    Array                       *unsat_soft_clauses;
    Array                       *sat_soft_clause_with_false_literal;//softclauses with 0<sat_num<literal_num, from which swap operation are choosen
    Array                       *contain_bool_unsat_soft_clauses;//unsat soft clause with at least one boolean var
    inline void                 sat_a_soft_clause(uint64_t clause_idx){unsat_soft_clauses->delete_element((int)clause_idx);contain_bool_unsat_soft_clauses->delete_element((int)clause_idx);};
    inline void                 unsat_a_soft_clause(uint64_t clause_idx){unsat_soft_clauses->insert_element((int)clause_idx);
                                 if(_soft_clauses[clause_idx].bool_literals.size()>0)contain_bool_unsat_soft_clauses->insert_element((int)clause_idx);};
    int                         best_found_obj;
    bool                        update_best_obj();
    void                        add_swap_operation_soft(int &operation_idx);
    //basic information
    uint64_t                    _num_vars;
    ...
    void                        build_soft_lits(char * filename);
    bool                        build_new_hard_lits(char * filename,std::vector<std::vector<int>> &new_hard_clauses,bool is_with_hard);
    std::string                 delete_pa(std::string org);
    int                         is_pa(std::string org);
    ...
    int                         pick_critical_move_sat(__int128_t &best_value);
    ...
    void                        select_best_operation_from_vec_sat(int operation_idx,int &best_score,int &best_var_idx,__int128_t &best_value);
    ...
    int                         get_pseudo_hard_clause_weight() {return pseudo_hard_clause_weight;};
    //print
    void                        print_formula();
    ...
    int                         critical_score_sat_region(uint64_t var_idx,__int128_t change_value);
    ...
    //local search
    int                        local_search(bool is_or_hard);
};
}
