...
namespace nia
{
    ...
    bool ls_solver::update_best_obj()
    {
        ...
        {
            int cur_obj = _solution[obj_vars];
    ...
    int ls_solver::critical_score_sat(uint64_t var_idx, __int128_t change_value)
    {
        ...
        for (int vlt_idx = 0; vlt_idx < var->var_lit_terms.size(); vlt_idx++)
        {
            ...
            {
                curr_lit_delta_new += coff * (std::pow((long long)future_value, vlt->exponent) - std::pow((long long)_solution[var_idx], vlt->exponent));
            }
        ...
        if (critical_score == 0) critical_score += 100000;
        critical_score += (change_value  * (-1) *  obj_weight);
        return critical_score;
    }

    void ls_solver::select_best_operation_from_sat(int operation_idx, int &best_score, int &best_var_idx, __int128_t &best_value)
    {
        bool BMS;
        int cnt, score;
        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            score = critical_score_sat(operation_var_idx, operation_change_value);
    ...
    int ls_solver::pick_obj_move(__int128_t &best_value)
    {
        if (_solution[obj_vars] == _vars[obj_vars].obj_low_bound)
        {
            // std::cout << "optimal found " << print_128(_solution[obj_vars]);
            std::cout << print_128(_solution[obj_vars]);
            exit(0);
        }
        int best_score(0), best_var_idx(-1), operation_idx(0);
        variable *var = &(_vars[obj_vars]);
        clause *c;
        lit *l;
        size_t index;
        var_lit_term * vlt; 
        int l_var_lit_term_num;
        __int128_t coff = 0;
        uint64_t var_idx_curr;
        __int128_t lit_delta;

        obj_var_term ovt = _obj_var_terms[0];
        if (ovt.exponent > 2) return -1;
        else if (ovt.exponent == 2)
        {
            int a = ovt.coffs[0];
            int b = ovt.coffs[1];
            insert_operation(ovt.var_idx, b/(-2*a) , operation_idx, false);
        }
        ...
        for (int cls_idx : var->clause_idxs)
        {
            ...
            for (int l_idx : c->literals)
            {
                ...
                if ( (l_idx < 0 && l->is_true) || (l_idx < 0 && !l->is_true) || l->is_equal) continue;
                auto it = std::find_if(l->var_lit_terms.begin(), l->var_lit_terms.end(),std::bind(varIdxEquals, std::placeholders::_1, obj_vars));
                if (it == l->var_lit_terms.end()) continue;
                else index = std::distance(l->var_lit_terms.begin(), it);
                l_var_lit_term_num = (int)l->var_lit_terms.size();
                var_idx_curr = l->var_lit_terms[index].var_idx;//= obj var index
                // std::cout << "here1" << std::endl;
                if (var_idx_curr != obj_vars) std::cout << "error with obj != expected" << std::endl;
                lit_delta = l->delta;
                int total_term_value = 0; // the value of terms containing the var
                bool too_high_exp = false; // if the exponent of var is larger than 2, then the var should be modified
                int coffs[2] = {0, 0};     // the coff = sum(term_coff*vlt_coff) ( 1 * x1*x2 ) + ( -1 * x1 ) + ( -1 * x3 ) for x1 the coff=(1*x2)+(-1)
                for (int vlt_idx = index; vlt_idx < l_var_lit_term_num; vlt_idx++)
                {
                    vlt = &(l->var_lit_terms[vlt_idx]);
                    if (vlt->var_idx != var_idx_curr)
                        break; // enter a new var
                    if (vlt->exponent > 2)
                        too_high_exp = true;
                    if (too_high_exp)
                        break;                                                                                             // if too high exponent, the operations will not be inserted
                    ...
                    if ((vlt_idx == l_var_lit_term_num - 1) || (var_idx_curr != l->var_lit_terms[vlt_idx + 1].var_idx))
                    {
                        ...
                        {
                            insert_operation(var_idx_curr, -1, operation_idx, false);
                            if (_vars[var_idx_curr].low_bound != -max_int && _vars[var_idx_curr].low_bound - _solution[var_idx_curr] > INT32_MIN)
                                insert_operation(var_idx_curr, _vars[var_idx_curr].low_bound - _solution[var_idx_curr], operation_idx, false);
                            continue;
                        } // if coff==0, change the var by 1
                        else if (coffs[1] == 0)
                        {
                            if (l_idx > 0 && devide_sat((-lit_delta), coffs[0]) < 0 && devide_sat((-lit_delta), coffs[0]) > INT32_MIN)
                            {
                                insert_operation(var_idx_curr, devide_sat((-lit_delta), coffs[0]), operation_idx, false);
                            
                            } // the delta should be <=0, while it is now >0, so the change value should add -delta/coff
                            else if (l_idx < 0 && devide_sat((1 - lit_delta), coffs[0]) < 0 && devide_sat((1 - lit_delta), coffs[0]) > INT32_MIN)//3x > 17 -> 0 >= 17 + 1 -3x = -(3x-17) +1
                            {
                                insert_operation(var_idx_curr, devide_sat((1 - lit_delta), coffs[0]), operation_idx, false);
                            } 
                        } // high coff is 1
                        else
                        {
                            add_coff(var_idx_curr, false, vlt_idx, operation_idx, coffs[1], coffs[0], l->delta - total_term_value);
                        }
                    }
                }
            }
        }
        //do selection and random walk 5.3
        select_best_operation_from_sat(operation_idx, best_score, best_var_idx, best_value);
        ...
        // std :: cout<<  _vars[obj_vars].obj_low_bound <<std::endl;
        // std::cout<<" first: "<<print_128(_solution[obj_vars])<<std::endl;
        // exit(0);
        if (ovt.exponent == 2)
        {
            int new_assignment = _solution[obj_vars] + 1;
            float old_value = _solution[obj_vars] * _solution[obj_vars] * ovt.coffs[0] + _solution[obj_vars] * ovt.coffs[1];
            float new_value = new_assignment * new_assignment * ovt.coffs[0] + new_assignment * ovt.coffs[1];
            if (new_value < old_value && _solution[obj_vars] - 1 >= _vars[obj_vars].obj_low_bound) 
            {
                critical_move(obj_vars, 1);
            }
            else if (new_value > old_value && _solution[obj_vars] + 1 <= _vars[obj_vars].obj_upper_bound)
            {
                critical_move(obj_vars, -1);
            }
        }
        else if (ovt.exponent == 1)
        {
            if (ovt.coffs[0] > 0)
            {
                if (_solution[obj_vars] > INT32_MAX) return -1;
                if (_solution[obj_vars] - 1 >= _vars[obj_vars].obj_low_bound)
                {
                    critical_move(obj_vars, -1);
                }
            }
            else if (ovt.coffs[0] < 0)
            {
                if (_solution[obj_vars] < INT32_MIN) return -1;
                if (_solution[obj_vars] + 1 <= _vars[obj_vars].obj_upper_bound)
                {
                    critical_move(obj_vars, 1);
                }
            }
    ...
    __int128_t ls_solver::devide_sat(__int128_t a, __int128_t b)
    {
        __int128_t abs_a = abs_128(a);
        __int128_t abs_b = abs_128(b);
        __int128_t up_round = abs_a / abs_b;//-3X <= 17  x = 5  delta = -32 32/3 = 10
    ...
    void ls_solver::add_coff(uint64_t var_idx_curr, bool use_tabu, int lit_idx, int &operation_idx, int coff_1)
    {
        ...
        else
        {
            if (lit_idx > 0)
            {
                insert_operation(var_idx_curr, devide((-lit_delta), coff_1), operation_idx, use_tabu);
            } // the delta should be <=0, while it is now >0, so the change value should add -delta/coff
            else
            {
                insert_operation(var_idx_curr, devide((1 - lit_delta), coff_1), operation_idx, use_tabu);
    ...
    void ls_solver::add_coff(uint64_t var_idx_curr, bool use_tabu, int lit_idx, int &operation_idx, int a, int b, int c)
    {
        ...
        if ((D > 0 && left_up == right_up) || (D == 0 && left_up != left_low) || (D < 0))
        {
            return;
        } // no integer solution on the other side
        if (l->is_equal)
        {
            if (lit_idx > 0)
            {
        ...
        else
        {
            if (lit_idx > 0)
            {
                if (D == 0 && a > 0 && left_up == left_low)
                {
                    insert_operation(var_idx_curr, left_up - _solution[var_idx_curr], operation_idx, use_tabu);
                } // the only 0 solution is left_up
                else if (D > 0)
                {
                    if (a > 0)
                    {
                        if (a * left_up * left_up + b * left_up + c > 0)
                        {
                            left_up++;
                        }
                        if (a * right_low * right_low + b * right_low + c > 0)
                        {
                            right_low--;
                        }
                        insert_operation(var_idx_curr, left_up - _solution[var_idx_curr], operation_idx, use_tabu);
                    ...
                    {
                        if (a * left_low * left_low + b * left_low + c > 0)
                        {
                            left_low--;
                        }
                        if (a * right_up * right_up + b * right_up + c > 0)
                        {
                            right_up++;
                        }
    ...
    {
        if (is_sat && (var_idx == obj_vars) &&  (_solution[var_idx] + change_value < _vars[obj_vars].obj_low_bound) ) return;
        ...
        if (_record_frequency)
        {
            modify_frequency_in_unsat_clauses();
        }
        // step
        if (_vars[var_idx].is_nia)
        {
    ...
    __int128_t ls_solver::coff_in_term(uint64_t var_idx, uint64_t term_idx, int exponent)
    {
        ...
        if (_solution[var_idx] != 0)
        {
            ...
            {
                return _terms[term_idx].value / (std::pow((long long)_solution[var_idx], exponent));
            }
        ...
        else
        {
            ...
            for (var_exp &ve : _terms[term_idx].var_epxs)
            {
                ...
                {
                    coff *= std::pow((long long)_solution[ve.var_index], ve.exponent);
                }
                if (coff == 0)
                {
                    break;
        ...
        if (var_in_long_term->is_in_array((int)var_idx))
        {
            for (term_exp &te : _vars[var_idx].term_exps)
            {
                ...
                if (t.var_epxs.size() > 2 || t.var_epxs[0].exponent > 1)
                {
                    ...
                    {
                        future_term_value = coff_in_term(var_idx, te.term_idx, te.exponent) * std::pow((long long)(_solution[var_idx] + change_value), te.exponent);
                    }
    ...
    void ls_solver::move_update(uint64_t var_idx, __int128_t change_value)
    {
        ...
        for (term_exp &te : var->term_exps)
        {
            ...
            {
                _terms[te.term_idx].value = term_coff * std::pow((long long)(future_value), te.exponent);
            }
        }
        ...
        for (int vlt_idx = 0; vlt_idx < var->var_lit_terms.size(); vlt_idx++)
        {
            ...
            {
                curr_lit_delta_new += lit_coff * (std::pow((long long)future_value, vlt->exponent) - std::pow((long long)_solution[var_idx], vlt->exponent));
            }
            if ((vlt_idx == var->var_lit_terms.size() - 1) || (curr_lit_idx != var->var_lit_terms[vlt_idx + 1].lit_idx))
            {
                if (_lits[curr_lit_idx].is_equal)
    ...
    bool ls_solver::check_solution()
    {
        ...
        for (int term_idx = 0; term_idx < _terms.size(); term_idx++)
        {
            ...
            for (var_exp &ve : t->var_epxs)
            {
                ...
                {
                    new_term_value *= std::pow((long long)_solution[ve.var_index], ve.exponent);
                }
                if (new_term_value == 0)
                {
                    break;
    ...
    bool ls_solver::local_search()
    {
        // if (is_high_coff())
        // {
        //     std::cout << "not found obj3" << std::endl;
        //     return false;
        // }
        if (build_unsat)
            return false;
        ...
        {
            if (0 == unsat_clauses->size() && !is_omt)
            {
                // check_solution();
                // up_bool_vars();
                std::cout << "sat but not omt" <<std::endl;
                return true;
            }
            ...
            if (no_improve_cnt > 500000 && is_sat == 0)
            {
                initialize();
                no_improve_cnt = 0;
            } // restart
            ...
            {
                /* code */
                flipv = pick_obj_move(change_value);
                if (flipv != -1) critical_move(flipv, change_value);
                if(0 == unsat_clauses->size()) 
                    update_best_obj();
            }
            else if (is_in_bool_search)
            {
        ...
        if(is_sat==1) std::cout<< print_128(best_found_obj) << std::endl;
        else std::cout << "not found obj" << std::endl;
        return false;
    }
