...
{
    // print
    void ls_solver::test_error()
    {
        std::cout<<unsat_clauses->size()<<"  "<<unsat_soft_clauses->size()<<std::endl;
        for (int i = 0; i < unsat_clauses->size(); i++)
        {
            std::cout<<unsat_clauses->element_at(i)<<std::endl;
        }
        std::cout<<"soft: "<<std::endl;
        for (int i = 0; i < unsat_soft_clauses->size(); i++)
        {
            std::cout<<unsat_soft_clauses->element_at(i)<<std::endl;
        }
    }

    void ls_solver::print_term_reduce(term &t)
    {
    ...
    void ls_solver::print_formula_with_soft()
    {
        ...
        for (int i = 0; i < _num_clauses; i++)
        {
            ...
            for (int l_idx : cl->literals)
            {
                ...
                print_literal_reduce(_lits[std::abs(l_idx)]);
            }
            std::cout << "\n";
        }
        for (int i = 0; i < _num_soft_clauses; i++)
        {
            soft_clause *scl = &(_soft_clauses[i]);
            std::cout << i << "\n";
    ...
    void ls_solver::print_term(term &t)
    {
        ...
        for (int i = 0; i < t.var_epxs.size(); i++)
        {
            ...
            std::cout << _tmp_vars[ve->var_index].var_name;
            if (ve->exponent > 1)
            {
                std::cout << "^" << ve->exponent;
            }
        }
    }
    ...
    void ls_solver::print_lit_pbs(lit &l)
    {
        ...
        for (int i = 0; i < l.coff_terms.size(); i++)
        {
            degree += l.coff_terms[i].coff;
        }
        std::cout << _num_vars + 1 << " " << print_128(degree) << " ";
        for (int i = 0; i < l.coff_terms.size(); i++)
        {
            std::cout << print_128(l.coff_terms[i].coff) << " " << -(l.coff_terms[i].term_idx + 1) << " ";
        }
        std::cout << "0\n";
    }
    ...
    { // reconstruct the solution by pop the stack
        for (variable &var : _tmp_vars)
        {
            if (var.is_nia && name2var.find(var.var_name) == name2var.end())
            {                                                                        // if it is an nia var and it is not in the formula
        ...
        {
            if (!_resolution_vars[i].is_nia && _resolution_vars[i].up_bool == 0)
            {
            ...
            for (int l_idx : cl.literals)
            {
                ...
                if (l->is_nia_lit)
                {
                    if (!lit_appear[std::abs(l_idx)])
                    {
                        ...
                        {
                            if (!term_appear[cf.term_idx])
                            {
                                sat_flag = true;
                                break;
                            } // if the term does not exist, the nia lit can be true
                            else
                            {
                                delta += (cf.coff * _terms[cf.term_idx].value);
                            }
                        }
                        if (!l->is_equal)
                        {
