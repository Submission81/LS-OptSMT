...
namespace nia
{
    int ls_solver::critical_score_sat_region(uint64_t var_idx, __int128_t change_value)
    {
        ...
        {
            term_coffs[term_idx] = coff_in_term(var_idx, term_idx);
        ...
        soft_clause *sc;
        for (int cls_idx : var->soft_clause_idxs)
        {
            sc = &(_soft_clauses[cls_idx]);
            int clause_sat_count_new = sc->sat_count;
            int degree=sc->nia_literals.size();
            for(int i=0;i<sc->literals.size();i++)
            {
                int v_lit=sc->literals[i];
                clause_sat_count_new += (v_lit > 0) ? _lit_make_break[v_lit] : (-_lit_make_break[-v_lit]);
                if(v_lit>0) critical_score+=_lit_make_break[v_lit]*sc->literal_weight[i];
                else critical_score-=_lit_make_break[-v_lit]*sc->literal_weight[i];
            }
            // for (int v_lit : sc->literals)
            // {
            //     clause_sat_count_new += (v_lit > 0) ? _lit_make_break[v_lit] : (-_lit_make_break[-v_lit]);
            // }
            // int distance=clause_sat_count_new-sc->sat_count;
            // critical_score+=sc->weight*distance;
            if (sc->sat_count == degree && clause_sat_count_new != degree)
            {
                critical_score -= sc->weight;
            }
            else if (sc->sat_count != degree && clause_sat_count_new == degree)
            {
                critical_score += sc->weight;
            }
        }
        ...
        for (uint64_t l_idx : var->soft_literal_idxs)
        {
            _lit_make_break[l_idx] = 0;
        }
        if (is_sat)
    ...
    void ls_solver::select_best_operation_from_vec_sat(int operation_idx, int &best_score, int &best_var_idx, __int128_t &best_value)
    {
        bool BMS;
        int cnt, score;
        ...
        for (int i = 0; i < cnt; i++)
        {
            ...
            score = critical_score_sat_region(operation_var_idx, operation_change_value);
    ...
    int ls_solver::pick_critical_move_sat(__int128_t &best_value)
    {
        ...
        if(unsat_clauses->size()==0)//不确定是不是留下
        // if(1)
        {
            for (int i = 0; i < unsat_soft_clauses->size(); i++)
            {
                soft_clause *scl = &(_soft_clauses[unsat_soft_clauses->element_at(i)]);
                for (int l_idx : scl->nia_literals)
                {
                    if (l_idx * _lits[std::abs(l_idx)].is_true > 0) continue;
                    add_operation_from_false_lit(true, l_idx, operation_idx);
                }
            }
        }
        ...
        select_best_operation_from_vec_sat(operation_idx, best_score, best_var_idx, best_value);
        // if there is untabu decreasing move
        if (best_var_idx != -1)
        {
            if(debug_flag==1) std::cout<<"sat walk:  "<<best_var_idx<<" "<<print_128(best_value)<<std::endl;
            return best_var_idx;
        }
        return -1;
    }
        ...
        if(is_sat==1)
        {
            ...
            // if(0)
            {
                for (int i = 0; i < unsat_soft_clauses->size(); i++)
                {
                    ...
                    for (int l_idx : unsat_scl->nia_literals)
                    {
                        if (l_idx * _lits[std::abs(l_idx)].is_true > 0||unsat_scl->literal_weight[l_idx]>unsat_scl->org_weight*1.5) continue;
                        unsat_scl->literal_weight[l_idx]++;
                        // std::cout<<unsat_scl->literal_weight[l_idx]<<"     "<<l_idx<<"   "<<unsat_scl->literal_weight.size()<<std::endl;
                    }
                }
            }
            else 
            {
                for (int i = 0; i < unsat_soft_clauses->size(); i++)
                {
                    ...
                    for(int j=0;j<unsat_scl->nia_literals.size();j++)
                    {
                        int l_idx=unsat_scl->nia_literals[j];
                        if (l_idx * _lits[std::abs(l_idx)].is_true > 0||unsat_scl->literal_weight[j]>600) continue;//20
                        else unsat_scl->literal_weight[j]++;
                        // std::cout<<unsat_scl->literal_weight[j]<<"     "<<j<<"   "<<unsat_scl->literal_weight.size()<<std::endl;
                    }
                  
                    // for (int l_idx : unsat_scl->nia_literals)
                    // {
                    //     if (l_idx * _lits[std::abs(l_idx)].is_true > 0||unsat_scl->literal_weight[l_idx]>600) continue;
                    //     unsat_scl->literal_weight[l_idx]++;
                    //     // std::cout<<unsat_scl->literal_weight[l_idx]<<"     "<<l_idx<<std::endl;
                    // }
    ...
    void ls_solver::smooth_clause_weight()
    {
        ...
        for (int i = 0; i < _num_soft_clauses; i++)
        {
            if (_soft_clauses[i].weight > 1 && !unsat_soft_clauses->is_in_array(i))
            {
                ...
                // for (int l_idx : _soft_clauses[i].nia_literals)
                // {
                //     if(_soft_clauses[i].literal_weight[l_idx]>1) _soft_clauses[i].literal_weight[l_idx]--;
                // }
                for(int j=0;j<_soft_clauses[i].nia_literals.size();j++)
                {
                    if(_soft_clauses[i].literal_weight[j]>1) _soft_clauses[i].literal_weight[j]-=1;
                }
                // if (_soft_clauses[i].sat_count == 1 && !_lits[std::abs(_soft_clauses[i].watch_lit_idx)].is_nia_lit)
                // {
                //     __int128_t watch_lit_idx = _lits[std::abs(_soft_clauses[i].watch_lit_idx)].delta;
                //     _vars[watch_lit_idx].score+=1;
                // }
            }
            // for(int j=0;j<_soft_clauses[i].nia_literals.size();j++)
            // {
            //     int l_idx=_soft_clauses[i].nia_literals[j];
            //     if (l_idx * _lits[std::abs(l_idx)].is_true < 0) continue;//20
            //     if(_soft_clauses[i].literal_weight[j]>1) _soft_clauses[i].literal_weight[j]-=1;
            // }
        }
    }
    ...
    void ls_solver::no_operation_random_walk()
    {
        ...
        if(unsat_clauses->size()!=0)
        {
            ...
            lit_idx = cp->literals[mt() % cp->literals.size()];
        }
        else if(unsat_soft_clauses->size()!=0)
        {
            while(1)
            {
                soft_clause *scp = &(_soft_clauses[unsat_soft_clauses->element_at(mt() % unsat_soft_clauses->size())]); // choose a random unsat clause
                lit_idx = scp->literals[mt() % scp->literals.size()];
                if (lit_idx * _lits[std::abs(lit_idx)].is_true < 0) break;
            }
        }
        ...
        if(debug_flag==1) std::cout<<"no operation walk:  "<<var_idx_curr<<" "<<print_128(future_solution - _solution[var_idx_curr])<<std::endl;
    ...
    void ls_solver::random_walk()
    {
        ...
        if (operation_idx_bool == 0 || (operation_idx > 0 && operation_idx_bool > 0 && !is_in_bool_search))
        {
            ...
            if(debug_flag==1) std::cout<<"random walk:  "<<best_var_idx_nia<<" "<<print_128(best_value_nia)<<std::endl;
        }
        // bool mode make move
        else
        {
    ...
    int ls_solver::pick_critical_move_bool()
    {
        ...
        if(unsat_clauses->size()==0)
        {
            ...
            {
                std::cout<<"bool in soft clauses break;";
                break;
                int clause_idx = contain_bool_unsat_soft_clauses->element_at(i);
                soft_clause *scl = &(_soft_clauses[clause_idx]);
            ...
            if ((vlt_idx == l_var_lit_term_num - 1) || (var_idx_curr != l->var_lit_terms[vlt_idx + 1].var_idx))
            {
                ...
                if (l->is_equal)
                {
                    if (lit_idx > 0)
                    {
                        if (lit_delta % coff == 0)
                        {
                            insert_operation(var_idx_curr, (-lit_delta / coff), operation_idx, use_tabu);
                        }
                ...
                else
                {
                    if (lit_idx > 0)
                    {
                        insert_operation(var_idx_curr, devide((-lit_delta), coff), operation_idx, use_tabu);
                    } // the delta should be <=0, while it is now >0, so the change value should add -delta/coff
                    else
                    {
                        insert_operation(var_idx_curr, devide((1 - lit_delta), coff), operation_idx, use_tabu);
    ...
    int ls_solver::pick_critical_move(__int128_t &best_value)
    {
        ...
        if (unsat_clauses->size() == 0)
        {
            int var_temp = pick_critical_move_sat(best_value);
            if (var_temp != -1) return var_temp;
        }
        int best_score(1), best_var_idx(-1), operation_idx(0);
        // best_score = -1000000;
        // determine the critical value from unsat sat, using tabu
        ...
        if (best_var_idx != -1)
        {
            if(debug_flag==1) std::cout<<"greedy walk:  "<<best_var_idx<<" "<<print_128(best_value)<<std::endl;
            return best_var_idx;
        }
        ...
        if (!sat_clause_with_false_literal->empty())
        {
            ...
            if (best_var_idx != -1)
            {
                if(debug_flag==1) std::cout<<"greedy walk  two level:  "<<best_var_idx<<" "<<print_128(best_value)<<std::endl;
                return best_var_idx;
            }
        }
        // if(unsat_clauses->size()==0&&!sat_soft_clause_with_false_literal->empty())
        // {
        //     operation_idx = 0;
        //     add_swap_operation_soft(operation_idx);
        //     // recover the false_lit
        //     false_lit_occur->clear();
        //     select_best_operation_from_vec(operation_idx, best_score, best_var_idx, best_value);
        //     if (best_var_idx != -1)
    ...
    void ls_solver::add_swap_operation_soft(int &operation_idx)
    {
        int clause_idx;
        soft_clause *scl;
        if (sat_soft_clause_with_false_literal->size() < 20)
        {
            for (int i = 0; i < sat_soft_clause_with_false_literal->size(); i++)
            {
                clause_idx = sat_soft_clause_with_false_literal->element_at(i);
                scl = &(_soft_clauses[clause_idx]);
                for (int l_idx : scl->nia_literals)
                {
                    if ((_lits[std::abs(l_idx)].is_true ^ l_idx) < 0)
                    {
        ...
        else
        {
            for (int i = 0; operation_idx < 20 && i < 50; i++)
            {
                clause_idx = sat_soft_clause_with_false_literal->element_at(mt() % sat_soft_clause_with_false_literal->size());
                scl = &(_soft_clauses[clause_idx]);
                for (int l_idx : scl->nia_literals)
                {
                    if ((_lits[std::abs(l_idx)].is_true ^ l_idx) < 0)
    ...
    void ls_solver::move_update(uint64_t var_idx, __int128_t change_value)
    {
        ...
        {
            sc = &(_soft_clauses[cls_idx]);
            int clause_sat_count_old = sc->sat_count;
            int watch_lit_idx_old = sc->watch_lit_idx;
            sc->sat_count = 0; // reset the sat count
            int degree=sc->literals.size();
            for (int l_idx : sc->literals)
            {
                if ((l_idx ^ _lits[std::abs(l_idx)].is_true) >= 0)
                {
                    sc->sat_count++;
                    // sc->watch_lit_idx = l_idx;
                }
            }
            // sat or unsat a clause, update the lit_in_unsat_clause
            if (sc->sat_count == degree && clause_sat_count_old != degree)
            {
                sat_a_soft_clause(cls_idx);
                _lit_in_unsat_soft_clause_num -= sc->literals.size();
                _bool_lit_in_unsat_soft_clause_num -= sc->bool_literals.size();
            }
            else if (sc->sat_count != degree && clause_sat_count_old == degree)
            {
                unsat_a_soft_clause(cls_idx);
                _lit_in_unsat_soft_clause_num += sc->literals.size();
                _bool_lit_in_unsat_soft_clause_num += sc->bool_literals.size();
            }
            // update the sat_clause_with_false_lit
            // if (sc->sat_count > 0 && sc->sat_count < sc->literals.size())
            // {
            //     sat_soft_clause_with_false_literal->insert_element((int)cls_idx);
            // }
            // else
            // {
            //     sat_soft_clause_with_false_literal->delete_element((int)cls_idx);
            // }
            // update the score of boolean vars
            // if (clause_sat_count_old == 0 && sc->sat_count > 0)
            // {
            //     for (int l_sign_idx : sc->bool_literals)
            //     {
            //         l = &(_lits[std::abs(l_sign_idx)]);
            //         _vars[l->delta].score -= sc->weight;
            //     }
            // }
            // else if (clause_sat_count_old == 1 && sc->sat_count > 1)
            // {
            //     l = &(_lits[std::abs(watch_lit_idx_old)]);
            //     if (!l->is_nia_lit)
            //     {
            ...
            //     }
            // }
            // else if (clause_sat_count_old > 0 && sc->sat_count == 0)
            // {
            //     for (int l_sign_idx : sc->bool_literals)
            //     {
            //         l = &(_lits[std::abs(l_sign_idx)]);
            ...
            // else if (clause_sat_count_old > 1 && sc->sat_count == 1)
            // {
            //     l = &(_lits[std::abs(sc->watch_lit_idx)]);
            //     if (!l->is_nia_lit)
            //     {
            //         _vars[l->delta].score -= sc->weight;
            //     }
    ...
    void ls_solver::move_update(uint64_t var_idx)
    {
        ...
        //soft new
        for (int i = 0; i < var->soft_clause_idxs.size(); i++)
        {
            std::cout<<"soft clauses has bool var"<<std::endl;
            break;
            cl_sign_idx = var->soft_clause_idxs[i];
            if ((_solution[var_idx] > 0 && var->bool_var_in_pos_clause[i]) || (_solution[var_idx] < 0 && !var->bool_var_in_pos_clause[i]))
            {
                make_break_in_clause = -1;
            } // true to false
            ...
            int clause_idx = std::abs(cl_sign_idx);
            soft_clause *scp = &(_soft_clauses[clause_idx]);
            // sat or unsat a clause
            ...
            int origin_watch_lit = scp->watch_lit_idx;
            ...
            // update the watch lit
            if (std::abs(origin_watch_lit) == v_lit_idx && scp->sat_count > 0)
            { // if the original watch lit exactly contains the var and the sat_count>0, set the new watch lit
                for (int l_idx : scp->literals)
                {
                    if ((l_idx ^ _lits[std::abs(l_idx)].is_true) >= 0)
                    {
                        scp->watch_lit_idx = l_idx;
                        break;
                    }
                }
            }
            ...
            {
                scp->watch_lit_idx = (cl_sign_idx > 0) ? (v_lit_idx) : (-v_lit_idx);
            } // if origin sat count==0, then set new watch lit
            // update the score of boolean vars
            if (make_break_in_clause > 0)
            {
            ...
            else if (make_break_in_clause < 0)
            {
                ...
                {
                    lit *l = &(_lits[std::abs(scp->watch_lit_idx)]);
                    if (!l->is_nia_lit)
                    {
                        _vars[l->delta].score -= scp->weight;
                    }
    ...
    bool ls_solver::check_solution()
    {
        ...
        {
            if (_lits[lit_idx].lits_index == 0 || !_lits[lit_idx].is_nia_lit)
            {
                continue;
            }
        ...
        int unsat_soft_num=0;
        for (uint64_t i = 0; i < _soft_clauses.size(); i++)
        {
            int sat_count = 0;
            soft_clause * scp = &(_soft_clauses[i]);
            for (int lit_idx : scp->literals)
            {
                __int128_t delta = _lits[std::abs(lit_idx)].delta;
                bool is_equal = _lits[std::abs(lit_idx)].is_equal;
                if (!_lits[std::abs(lit_idx)].is_nia_lit)
            ...
            if (sat_count != scp->sat_count)
            {
                std::cout << "soft sat count wrong at clause " << i << "\n";
            }
            if (sat_count != scp->literals.size())
            {
                unsat_soft_num++;
            }
        }
        if(unsat_soft_num!=unsat_soft_clauses->size()) std::cout<<"soft error"<<std::endl;
    ...
    bool ls_solver::update_best_obj()
    {
        ...
        if(0==unsat_clauses->size())
        {
            ...
            if(cur_obj<best_found_obj)
            {
                ...
                // check_solution();
                // std::cout<<"update: "<<unsat_soft_clauses->size()<<std::endl;
                return true;
            }
            else {
                return false;
    ...
    // local search
    int ls_solver::local_search(bool is_or_hard)
    {
        if(is_or_hard) _cutoff/=2;
        ...
        // test_error();
        // exit(0);
        // std::cout<<bool_var_vec.size()<<std::endl;
        for (_step = 1; _step < _max_step; _step++)
        {
            is_improve=0;
            // std::cout<<unsat_clauses->size()<<"  "<<unsat_soft_clauses->size()<<std::endl;
            ...
            if(0 == unsat_clauses->size()&&is_restart==0)
            {
                is_restart=-1;
                initialize();//restart
            }
            ...
            bool time_up_nia = (no_improve_cnt_nia * (_lit_in_unsat_clause_num+_lit_in_unsat_soft_clause_num) > 20* (_bool_lit_in_unsat_clause_num+_bool_lit_in_unsat_soft_clause_num));
            ...
            if (is_in_bool_search)
            {
                ...
                // std::cout<<flipv<<"   "<<print_128(change_value)<<std::endl;
                if(0==unsat_clauses->size())
                {
                    is_sat=1;
                    if(update_best_obj()) {is_improve=1;no_improve_cnt_bool=0;}
