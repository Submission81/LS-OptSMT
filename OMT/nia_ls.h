...
struct obj_var_term{
    uint64_t var_idx;
    int exponent;
    std::vector<int> coffs;
    obj_var_term(uint64_t _var_idx,int _exponent,std::vector<int> _coffs):var_idx(_var_idx),exponent(_exponent),coffs(_coffs){};
};
...
static bool varIdxEquals(const var_lit_term &vlt, uint64_t index) 
{
    return vlt.var_idx == index;
}
...
struct variable{
    ...
    int                         obj_low_bound = INT32_MAX;
    int                         obj_upper_bound = INT32_MIN;
};
...
public:
    ...
    //OMT
    std::vector<obj_var_term>    _obj_var_terms;
    bool                        is_omt = true;
    ...
    int                         obj_weight = 1;
    int                         is_sat = 0;
    __int128_t                         best_found_obj = INT32_MAX;
    int                         critical_score_sat(uint64_t var_idx,__int128_t change_value);
    void                        select_best_operation_from_sat(int operation_idx, int &best_score, int &best_var_idx, __int128_t &best_value);
    __int128_t                  devide_sat(__int128_t a, __int128_t b);
    int                         pick_obj_move(__int128_t &best_value);
    void                        get_objective(std::string in_string);
